...
namespace QLTKB
{
    public partial class frmLop : Form
    {
        ...
        {
            string sql = "Select * from tblLop where MaLop like '%" + txttim.Text + "%'";
            msds.DataSource = cn.taobang(sql);
        }
        ...
        private void btnluu_Click(object sender, EventArgs e)
        {
            if (themmoi == true)
            {
                ...
                {
                    if (MessageBox.Show("Bạn chưa nhập mã lớp !", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information) == DialogResult.OK)
                        txtma.Focus();
                    return;
                }
                if (txtten.Text.Trim().Length == 0)
                {
                    if (MessageBox.Show("Bạn chưa nhập tên lớp !", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information) == DialogResult.OK)
                        txtten.Focus();
                    return;
                }
