...
namespace QLTKB
{
    public partial class frmGiaoVien : Form
    {
        ...
        {
            string sql = "Select * from tblGiaoVien where MaGV like '%"+txttim.Text+"%'";
            msds.DataSource = cn.taobang(sql);
        }
        ...
        private void btnluu_Click(object sender, EventArgs e)
        {
            if (themmoi == true)
            {
                ...
                {
                    if (MessageBox.Show("Bạn chưa nhập mã giáo viên !", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information) == DialogResult.OK)
                        txtma.Focus();
                    return;
                }
                if (txtten.Text.Trim().Length == 0)
                {
                    if (MessageBox.Show("Bạn chưa nhập tên giáo viên !", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information) == DialogResult.OK)
                        txtten.Focus();
                    return;
                }
        ...
        private void msds_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            cbcm.Text = msds.Rows[dong].Cells[5].Value.ToString();
        }
